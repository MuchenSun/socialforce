...
class SimulatorMultiDest(object):
    ...
    """
    def __init__(self, initial_state, dest=None, ped_space=None, ped_ped=None, field_of_view=None, delta_t=0.4, tau=0.5):
    ...
    def update_dest(self, threshold=0.1):
        """update destination for pedestrian"""
        # loop through each pedastrian
        for i in range(self.state.shape[0]):
            # get state of the pedestrian
            curr_ped = self.state[i]
            # get current position and destination
            curr_pos = curr_ped[0:2]
            curr_dest = curr_ped[4:6]
            '''debug'''
            # print("curr_pos: ", curr_pos)
            # print("curr_dest:", curr_dest)
            # print("dist: ", np.sum((curr_pos-curr_dest)**2), end='\n\n')
            # decide whether update destination
            if np.sum((curr_pos-curr_dest)**2) > threshold**2:
                continue
            else:
                # choose a new destination
                new_dest = curr_dest
                while( (new_dest==curr_dest).all() ):
                    # randomly select a new destination
                    new_id = np.random.randint(0, len(self.dest))
                    new_dest = self.dest[new_id]
                self.state[i][4] = new_dest[0]
                self.state[i][5] = new_dest[1]

    def step(self):
        """Do one step in the simulation and update the state in place."""
        # if multiple destinations are provided, then update
